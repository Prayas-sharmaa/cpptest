...
def dashboard(request):
    ...
    return render(request, "dashboard.html", {
        "orders": orders,
        "inventory": inventory,
        "recipes": recipes,   # for simulator dropdown
        "low_stock": low_stock,
    })
...
def simulator_data(request):
    ...
    # Convert into simple name -> qty dict
    inv_after = {}
    for it in inv_after_list:
        name = it.get("name")
        if name:
            inv_after[name] = it.get("qty", it.get("quantity", 0))
